...
const tmdbTrendingParams = z.object({
    mediaType: z.enum(["all", "movie", "tv"]).describe("The type of media to search for (all, movie, or tv)"),
    timeWindow: z.enum(["day", "week"]).describe("The time window to search for trending items (day or week)"),
});
