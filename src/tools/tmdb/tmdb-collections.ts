...
export const tmdbSearchCollectionsTool = {
    ...
    execute: async (params: TmdbSearchCollectionsParams) => {
        ...
        try {
            ...
            const formatted = results.slice(0, 5) // Limit to top 5 results
                .map((collection, i) => dedent`
...
const tmdbCollectionDetailsParams = z.object({
    collectionId: z.number().describe("The ID of the collection to get details for."),
});
...
export const tmdbCollectionDetailsTool = {
    ...
    description: "Fetches detailed information about a specific movie collection by its ID using the TMDB service.",
    ...
    execute: async (params: TmdbCollectionDetailsParams) => {
        ...
        try {
            const collection = await tmdbService.getCollectionDetails(params.collectionId);

            if (!collection) {
                return `No collection found for ID ${params.collectionId}.`;
            }

            const formattedParts = collection.parts.map((movie, i) => dedent`
