...
const entertainmentSuggestionsParams = z.object({
	mediaType: z
		.enum(["movie", "tv"])
		.describe("The type of media to suggest (movie or tv)"),
	genre: z
		.string()
		.describe(
			"The genre of entertainment to suggest (e.g., 'Horror', 'Action')",
		),
	releaseYear: z
		.number()
		.optional()
	...
	minImdbRating: z
		...
		.max(10)
		.optional()
		.describe("Minimum IMDb rating (1-10)"),
});

type EntertainmentSuggestionsParams = z.infer<
	typeof entertainmentSuggestionsParams
>;
...
export const entertainmentSuggestionsTool = {
	...
	description:
		"Suggests entertainment based on genre, release year, and minimum IMDb rating",
	...
	execute: async (params: EntertainmentSuggestionsParams) => {
		...
		try {
			...
			const tmdbMovies = await tmdbService.discoverByGenre(
				params.mediaType,
				selectedGenre.id,
				params.releaseYear,
			);
			...
			for (const movie of tmdbMovies) {
				if (movie.imdbId) {
					...
					if (
						omdbDetails &&
						parseFloat(omdbDetails.rating) >= (params.minImdbRating ?? 0)
					) {
			...
			const formatted = suggestedMovies
