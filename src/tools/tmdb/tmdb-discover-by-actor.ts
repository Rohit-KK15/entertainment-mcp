...
const tmdbDiscoverByActorParams = z.object({
    actorId: z.number().describe("The ID of the actor to discover entertainment for. Use GET_TMDB_PERSON_SEARCH to find the actor ID."),
    mediaType: z.enum(["movie", "tv"]).describe("The type of media to discover (movie or tv)."),
...
export const tmdbDiscoverByActorTool = {
    ...
    description: "Discovers movies or TV shows by an actor's ID using the TMDB service.",
    ...
    execute: async (params: TmdbDiscoverByActorParams) => {
        ...
        try {
            const results = await tmdbService.discoverByActor(params.actorId, params.mediaType, params.releaseYear);

            if (!results.length) {
                return `No ${params.mediaType}s found for actor ID ${params.actorId}${params.releaseYear ? ` in ${params.releaseYear}` : ""}.`;
            }

            const formatted = results.slice(0, 5) // Limit to top 5 suggestions
                .map((item, i) => dedent`
